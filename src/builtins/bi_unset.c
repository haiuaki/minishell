/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bi_unset.c                                         :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: juljin <juljin@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/27 17:27:18 by juljin            #+#    #+#             */
/*   Updated: 2026/03/26 13:14:10 by juljin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

/*
 * Helper function to check every node inside the `t_env` list
 * to see if one of the given arguments to `unset` matches an existing variable.
 */
static void	unset_key_loop(t_env **head_ptr, char *key)
{
	t_env	*tmp;
	t_env	*prev;

	tmp = *head_ptr;
	prev = NULL;
	while (tmp)
	{
		if (ft_strcmp(tmp->key, key) == 0)
		{
			if (prev)
				prev->next = tmp->next;
			else
				*head_ptr = tmp->next;
			free_env_node(tmp);
			break ;
		}
		prev = tmp;
		tmp = tmp->next;
	}
}

/*
 * Helper function to process arguments and unset environment variables.
 */
static void	unset_args(t_shell *s, char **args)
{
	int	i;

	if (!args)
		return ;
	i = 0;
	while (args[i])
	{
		if (!key_isalnum(args[i]))
		{
			print_key_error("unset", args[i]);
			s->last_exit_code = 1;
			i++;
			continue ;
		}
		else if (ft_strcmp(args[i], "_") == 0)
		{
			i++;
			continue ;
		}
		unset_key_loop(&s->env_list, args[i]);
		i++;
	}
}

/* Implementation of the built-in command `unset` */
void	bi_unset(t_shell *s, t_cmd *cmd)
{
	s->last_exit_code = 0;
	if (!cmd->args[1])
		return ;
	unset_args(s, &cmd->args[1]);
}
/*
int	main(int ac, char *av[], char *envp[])
{
	t_env	*env_copy;
	t_env	*new_node;

	(void)ac;
	(void)av;
	env_copy = copy_env(envp);
	if (!env_copy)
		return (1);
	new_node = malloc(sizeof(t_env));
	new_node->key = ft_strdup("HELLO");
	new_node->value = NULL;
	new_node->next = NULL;
	printf("\n/////////////////// [BEFORE DELETION] ///////////////////\n");
	env_add_back(&env_copy, new_node);
	bi_env(env_copy);
	bi_unset("OLDPWD", &env_copy);
	printf("\n/////////////////// [AFTER DELETION] ///////////////////\n");
	bi_env(env_copy);
	return (0);
}
*/
